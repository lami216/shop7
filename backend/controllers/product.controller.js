...
const normalizeProductImages = (images) => {
        const normalized = Array.isArray(images)
                ? images.map(normalizeImageObject).filter(Boolean)
                : [];

        return normalized.map((image) =>
                image.fileId ? { url: image.url, fileId: image.fileId } : { url: image.url }
        );
};
...
const splitCurrentImages = (currentImages, retainedIds) => {
        const images = Array.isArray(currentImages)
                ? currentImages.map(normalizeImageObject).filter(Boolean)
                : [];
        return {
...
const deleteImagesFromStorage = async (images) => {
        const fileIdsToDelete = images
                .map((image) => (typeof image.fileId === "string" ? image.fileId : null))
                .filter(Boolean);
...
const orderRetainedImages = (existingImageIds, retainedImages) => {
        const lookup = retainedImages.reduce((accumulator, image) => {
                const key = getImageIdentifier(image);
                if (key) {
